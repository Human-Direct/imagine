...
/**
 * Class WeAreHiringTheme.
 */
class WeAreHiringTheme extends AbstractTheme implements AdaptiveThemeInterface
{
    /**
    ...
    private function loadThemes(): void
    {
        ...
        $files = new \RecursiveCallbackFilterIterator($directory, function ($current, $key, $iterator) use ($themeNS) {
            ...
            $isWeAreHiring = 0 === strpos($className, 'WeAreHiring');

            if (!$isTheme || !$isWeAreHiring || $current->isDir() || $iterator->hasChildren()) {
                return false;
            }
